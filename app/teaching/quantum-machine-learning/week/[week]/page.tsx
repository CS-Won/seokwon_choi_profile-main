...
import {
  QML_SCHEDULE,
  getQmlWeekRow,
} from "@/lib/teaching-quantum-machine-learning"
import Week0 from "@/components/teaching/qml/week-0"
import Week1 from "@/components/teaching/qml/week-1"
import Week2 from "@/components/teaching/qml/week-2"
import Week3 from "@/components/teaching/qml/week-3"
import Week4 from "@/components/teaching/qml/week-4"
import Week5 from "@/components/teaching/qml/week-5"

const BASE = "/teaching/quantum-machine-learning"
...
export function generateStaticParams() {
  return QML_SCHEDULE.map((row) => ({ week: row.week }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const row = getQmlWeekRow(week)
  if (!row) {
    return { title: "Week | Seokwon Choi" }
  }
  return {
    title: `week_${week}_page — ${row.topic} | Quantum Machine Learning (Spring 2025) | Seokwon Choi`,
    description: `${row.topic}. Week ${week} notes for Quantum Machine Learning (QIYA, Spring 2025).`,
  }
}

export default async function QmlWeekPage({ params }: Props) {
  const { week } = await params
  const row = getQmlWeekRow(week)
  if (!row) notFound()
  ...
  const idx = QML_SCHEDULE.findIndex((r) => r.week === week)
  const prev = idx > 0 ? QML_SCHEDULE[idx - 1] : null
  const next =
    idx >= 0 && idx < QML_SCHEDULE.length - 1 ? QML_SCHEDULE[idx + 1] : null
    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-3xl px-6 py-12 md:py-16">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Spring 2025 · QIYA — Lecturer: Seokwon Choi
        </p>
