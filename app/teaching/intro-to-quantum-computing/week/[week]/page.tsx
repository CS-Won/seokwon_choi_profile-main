...
import {
  IQC_SCHEDULE,
  getIqcWeekRow,
} from "@/lib/teaching-intro-to-quantum-computing"
import Week0 from "@/components/teaching/iqc/week-0"
import Week1 from "@/components/teaching/iqc/week-1"
import Week2 from "@/components/teaching/iqc/week-2"
import Week3 from "@/components/teaching/iqc/week-3"
import Week4 from "@/components/teaching/iqc/week-4"
import Week5 from "@/components/teaching/iqc/week-5"
import Week6 from "@/components/teaching/iqc/week-6"
import Week7 from "@/components/teaching/iqc/week-7"
import Week8 from "@/components/teaching/iqc/week-8"
import Week9 from "@/components/teaching/iqc/week-9"
import Week10 from "@/components/teaching/iqc/week-10"

const BASE = "/teaching/intro-to-quantum-computing"

const WEEK_CONTENT: Record<string, () => React.ReactElement> = {
  "0": Week0,
  "1": Week1,
  "2": Week2,
  "3": Week3,
...
export function generateStaticParams() {
  return IQC_SCHEDULE.map((row) => ({ week: row.week }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const row = getIqcWeekRow(week)
  if (!row) {
    return { title: "Week | Seokwon Choi" }
  }
  return {
    title: `week_${week}_page — ${row.topic} | Introduction to Quantum Computing (Winter 2024) | Seokwon Choi`,
    description: `${row.topic}. Materials for week ${week} of Introduction to Quantum Computing (Yonsei Premium Online Lecture).`,
  }
}

export default async function IntroQuantumComputingWeekPage({ params }: Props) {
  const { week } = await params
  const row = getIqcWeekRow(week)
  if (!row) notFound()
  ...
  const idx = IQC_SCHEDULE.findIndex((r) => r.week === week)
  const prev = idx > 0 ? IQC_SCHEDULE[idx - 1] : null
  const next =
    idx >= 0 && idx < IQC_SCHEDULE.length - 1 ? IQC_SCHEDULE[idx + 1] : null
    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-3xl px-6 py-12 md:py-16">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Winter 2024 · Yonsei Premium Online Lecture — Lecturer: Seokwon Choi
        </p>
