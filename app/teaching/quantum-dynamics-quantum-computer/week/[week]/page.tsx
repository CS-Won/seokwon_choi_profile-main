...
import Week0 from "@/components/teaching/qdyn/week-0"
import Week1 from "@/components/teaching/qdyn/week-1"
import Week2 from "@/components/teaching/qdyn/week-2"
import Week3 from "@/components/teaching/qdyn/week-3"
import Week4 from "@/components/teaching/qdyn/week-4"
import Week5 from "@/components/teaching/qdyn/week-5"

const BASE = "/teaching/quantum-dynamics-quantum-computer"
...
export function generateStaticParams() {
  return QDYN_SCHEDULE.map((row) => ({ week: row.week }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const row = getQdynWeekRow(week)
  if (!row) {
    return { title: "Week | Seokwon Choi" }
  }
  return {
    title: `week_${week}_page — ${row.topic} | Quantum Dynamics (Spring 2025) | Seokwon Choi`,
    description: `${row.topic}. Materials for week ${week} of Quantum Dynamics using Quantum Computer (QISCA).`,
  }
}

function WeekMaterialLinks({
  slideHref = "#",
  materialHref = "#",
}: {
  slideHref?: string
...
export default async function QuantumDynamicsWeekPage({ params }: Props) {
  const { week } = await params
  const row = getQdynWeekRow(week)
  if (!row) notFound()
  ...
  const idx = QDYN_SCHEDULE.findIndex((r) => r.week === week)
  const prev = idx > 0 ? QDYN_SCHEDULE[idx - 1] : null
  const next = idx >= 0 && idx < QDYN_SCHEDULE.length - 1 ? QDYN_SCHEDULE[idx + 1] : null
      ...
      <div className="mx-auto max-w-3xl px-6 py-12 md:py-16">
        ...
        <p className="font-mono text-xs text-muted-foreground">
          <Link href={BASE} className="text-accent hover:underline">
            Quantum Dynamics using Quantum Computer
          </Link>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Spring 2025 · QISCA — Lecturer: Seokwon Choi
        </p>
